...
- [x] 1. Set up `ConnectionPool` singleton (`db/pool.py`)
  - [x] 1.1 Create `services/api/src/omni_modal/db/__init__.py` (empty) and `services/api/src/omni_modal/db/pool.py`
  ...
  - [x]* 1.2 Write property test for `ConnectionPool` singleton identity (Property 3)
  ...
  - [x]* 1.3 Write property test for pool env-configured sizes (Property 4)
...
- [x] 2. Implement `QueryCache` (`qa/cache.py`)
  - [x] 2.1 Create `services/api/src/omni_modal/qa/cache.py`
  ...
  - [x]* 2.2 Write property test for cache key determinism and collision resistance (Property 5)
  ...
  - [x]* 2.3 Write property test for cache hit bypasses DB (Property 6)
  ...
  - [x]* 2.4 Write property test for cache miss stores result round-trip (Property 7)
  ...
  - [x]* 2.5 Write property test for tenant cache eviction completeness (Property 8)
  ...
  - [x]* 2.6 Write property test for cache size bounded by maxsize (Property 9)
...
- [x] 3. Implement `BatchEmbedder` (`ingestion/batch_embedder.py`)
  - [x] 3.1 Create `services/api/src/omni_modal/ingestion/batch_embedder.py`
  ...
  - [x]* 3.2 Write property test for batch partitioning correctness (Property 10)
  ...
  - [x]* 3.3 Write property test for batch insert row-count invariant (Property 11)
  ...
  - [x]* 3.4 Write property test for idempotent re-ingestion (Property 12)
  ...
  - [x]* 3.5 Write property test for connection released per batch (Property 16)
...
- [x] 4. Implement `BackgroundWorker` and `AsyncIngestionQueue`
  - [x] 4.1 Create `services/api/src/omni_modal/ingestion/worker.py`
  ...
  - [x] 4.2 Create `services/api/src/omni_modal/ingestion/async_queue.py`
  ...
  - [x]* 4.3 Write property test for job failure records full error context (Property 1)
  ...
  - [x]* 4.4 Write property test for at-most-one-concurrent job (Property 2)
  ...
  - [x]* 4.5 Write property test for watchdog restarts crashed worker (Property 15)
...
- [x] 5. Update `PgVectorChunkRetriever` to accept pool and cache
  - [x] 5.1 Modify `services/api/src/omni_modal/qa/retrieval.py`
...
- [x] 6. Wire all components in `main.py`
  - [x] 6.1 Modify `services/api/src/omni_modal/main.py`
...
- [x] 7. Checkpoint — core integration
  - Ensure all tests pass, ask the user if questions arise.
  - Run `python -m unittest discover -s services/api/tests`

- [x] 8. Add HNSW tuning migration
  - [x] 8.1 Create `packages/db/drizzle/0002_perf.sql`
...
- [x] 9. Implement benchmark harness (`benchmark/__main__.py`)
  - [x] 9.1 Create `services/api/src/omni_modal/benchmark/__init__.py` (empty) and `services/api/src/omni_modal/benchmark/__main__.py`
  ...
  - [x]* 9.2 Write property test for benchmark result serialisation completeness (Property 13)
...
- [x] 10. Add ADK workflow timeout property test
  - [x]* 10.1 Write property test for workflow total timeout enforcement (Property 14)
...
- [x] 11. Add integration tests
  - [x]* 11.1 Write integration test: `/health` responsive during background ingestion
  ...
  - [x]* 11.2 Write integration test: retriever uses pool not `psycopg.connect`
